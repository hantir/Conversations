...
public class FollowingActivity extends AppCompatActivity {
    ...
    private void getAllUsers() {
        ...
        gp.findInBackground(new FindCallback<ParseUser>() {
            public void done(List<ParseUser> uList, ParseException e) {
                if (e == null) {
                    if (uList.size() > 0) {
                        adapterUsers = new AdapterUsers(FollowingActivity.this, uList);
                        users_rv.setAdapter(adapterUsers);
                        findViewById(R.id.progressBar).setVisibility(View.GONE);
                        users_rv.setVisibility(View.VISIBLE);
                        findViewById(R.id.nothing).setVisibility(View.GONE);
    ...
    private void checkFollowing(){
        ...
        flwing.whereEqualTo("fromObj",ParseObject.createWithoutData(ParseUser.class, userId));
        flwing.whereNotEqualTo("toObj",ParseObject.createWithoutData(ParseUser.class, userId));
        flwing.findInBackground(new FindCallback<Follow>() {
            public void done(List<Follow> flList, ParseException e) {
                if (e == null) {
                    if (flList.size() > 0) {
                        followingList.clear();
                        for (Follow flobj : flList) {
                            followingList.add(flobj.getToObj().getObjectId());
                        }
                        getAllUsers();
                    }else {
                        findViewById(R.id.progressBar).setVisibility(View.GONE);
