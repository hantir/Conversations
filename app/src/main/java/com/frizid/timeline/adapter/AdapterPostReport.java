...
public class AdapterPostReport extends RecyclerView.Adapter<AdapterPostReport.MyHolder>{
    ...
    public AdapterPostReport(Context context, List<Post> posts) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
                ...
                if (e == null) {
                    ...
                    holder.dp.setOnClickListener(v -> {
                        if (!posts.get(position).getObjectId().equals(ParseUser.getCurrentUser())){
                            ...
                            intent.putExtra("hisUID", posts.get(position).getUserObj().getObjectId());
        ...
        //Time
        long lastTime = posts.get(position).getUpdatedAt().getTime();
            ...
            public void done(PostExtra peobj, ParseException e) {
                if (e == null) {
                    if (peobj.isDataAvailable()) {
                        ...
                        if (peobj.getPrivacy().equals("No one")){
                            ...
                            gpc.whereNotEqualTo("toObj", ParseUser.getCurrentUser());
        ...
        if (type.equals("gif")){
            holder.mediaView.setVisibility(View.VISIBLE);
            Glide.with(context).load(posts.get(position).getUrl()).thumbnail(0.1f).into(holder.mediaView);
        }
        ...
        vvq.countInBackground(new CountCallback() {
            public void done(int count, ParseException e) {
                if (e == null) {
                    if (count > 0) {
                        holder.layout.setVisibility(View.VISIBLE);
                        holder.viewsLayout.setVisibility(View.VISIBLE);
                        holder.noViews.setText(String.valueOf(count));
                    }else {
                            ...
                            public void done(List<PostReaction> rList, ParseException e) {
                                if (e == null) {
                                    for (PostReaction rListObj : rList){
                                        String value = rListObj.getValue();
                                        if (value.equals("thumb")) holder.thumb.setVisibility(View.VISIBLE);
        ...
        sharePop.setOnMenuItemClickListener(item -> {
            ...
            if (item.getItemId() == 1){
                ...
                intent.putExtra("uri", posts.get(position).getText());
                holder.itemView.getContext().startActivity(intent);
            }
            if (item.getItemId() == 2){
                //Intent intent = new Intent( holder.itemView.getContext(), SendToGroupActivity.class);
                //intent.putExtra("type", "post");
                //intent.putExtra("uri", posts.get(position).getpId());
                //holder.itemView.getContext().startActivity(intent);
            }
            return false;
        });
        ...
        morePop.setOnMenuItemClickListener(item -> {
            ...
            if (item.getItemId() == 9){
                ...
                intent.putExtra("id", posts.get(position).getObjectId());
                context.startActivity(intent);
            }
            if (item.getItemId() == 2){
               if (type.equals("text") || type.equals("bg") || type.equals("gif")){
                ...
                if (type.equals("text")){
                    ...
                    ClipboardManager clipboard = (ClipboardManager) context.getSystemService(Context.CLIPBOARD_SERVICE);
                    ClipData clip = ClipData.newPlainText("text", posts.get(position).getText() + " " + posts.get(position).getUrl());
                    clipboard.setPrimaryClip(clip);
                ...
                pobj.setPostObj(posts.get(position));
                ...
                pq.getInBackground(posts.get(position).getObjectId(), new GetCallback<Post>() {
        ...
        holder.itemView.setOnClickListener(v -> {
            ...
            popupMenu.setOnMenuItemClickListener(item -> {
                ...
                if (id == 1) {
                    ...
                    uq.getInBackground(posts.get(position).getUserObj().getObjectId(), new GetCallback<ParseUser>() {
                        public void done(ParseUser object, ParseException e) {
                            if (e == null) {
                    ...
                    nq.setPostObj(posts.get(position));
                    nq.setPUserObj( posts.get(position).getUserObj());
                    nq.setNotification( "You have got a warning by the admin");
                    nq.setSUserObj( ParseUser.getCurrentUser());
                    nq.saveInBackground();
                    ParseUser uobj = ParseObject.createWithoutData(ParseUser.class,posts.get(position).getUserObj().getObjectId());
                    uobj.increment("count");
                    uobj.saveInBackground();
                    notify = true;
                    if (notify){
                        App.sendNotification( posts.get(position).getUserObj().getObjectId(), uobj.getString("name").toString(), "You have got a warning by the admin");
                ...
                if (id == 2) {
                    ParseQuery<ReportPost> rqd = ParseQuery.getQuery(ReportPost.class);
                    rqd.whereEqualTo("postObj", posts.get(position));
                    rqd.whereEqualTo("userObj", ParseUser.getCurrentUser());
                    rqd.getFirstInBackground(new GetCallback<ReportPost>() {
                        public void done(ReportPost robj, ParseException e) {
                            if (e == null) {
                ...
                if (id == 3) {
                    Intent intent = new Intent(context, UserProfileActivity.class);
                    intent.putExtra("hisUID", posts.get(position).getObjectId());
                    context.startActivity(intent);
                }

                if (id == 4){
                    ParseQuery<Post> rqd = ParseQuery.getQuery(Post.class);
                    rqd.getInBackground(posts.get(position).getObjectId(), new GetCallback<Post>() {
                        public void done(Post robj, ParseException e) {
                            if (e == null) {
                                robj.deleteInBackground();
                            }
                            else {
    ...
    private void addToHisNotification(String hisUid, String message, String postId){
